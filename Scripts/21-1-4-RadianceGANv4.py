...
from numpy import asarray, savez_compressed, load, zeros, ones, hstack, expand_dims
...
def load_images(path, size=(256,256)):
    src_list, tar_list, lat_list, ghi_list = list(), list(), list(), list()
    #enumerate filenames in directory, assume all are images
    for location in listdir(PATH):
        # dataset paths
        src_path = PATH + location +"/plans/"
        tar_path = PATH + location + "/daysim/"
        ...
        for filename in listdir(tar_path):
            ...
            if location == "Ankara":
                lat_list.append(ankara_loc[0])
                ghi_list.append(ankara_loc[1])
            if location == "Brisbane":
                lat_list.append(brisbane_loc[0])
                ghi_list.append(brisbane_loc[1])
            if location == "London":
                lat_list.append(london_loc[0])
                ghi_list.append(london_loc[1])
            if location == "Nairobi":
                lat_list.append(nairobi_loc[0])
                ghi_list.append(nairobi_loc[1])
            if location == "Cairo":
                lat_list.append(cairo_loc[0])
                ghi_list.append(cairo_loc[1])    
    #normalize data
    lat_list = [(float(i)-min(lat_list))/(max(lat_list)-min(lat_list)) for i in lat_list]
    ghi_list = [(float(i)-min(ghi_list))/(max(ghi_list)-min(ghi_list)) for i in ghi_list]
    ...
    #stack arrays
    lat_list = expand_dims(lat_list, axis=1)
    ghi_list = expand_dims(ghi_list, axis=1)
    loc_array = hstack((lat_list, ghi_list))
    print(loc_array[250], loc_array[750], loc_array[1250], loc_array[1750])
    print(loc_array.shape)
    return [asarray(src_list), asarray(tar_list), loc_array]
...
# load dataset
[src_images, tar_images, loc_data] = load_images(PATH)
print("Loaded: ", src_images.shape, tar_images.shape, loc_data.shape)
# save as compressed numpy array
filename = "_datasets\\_radiancev4\\rad_256.npz"
savez_compressed(filename, src_images, tar_images, loc_data)
...
def define_discriminator(image_shape):
    ...
    #location data input
    location_data = Input(shape=(2))
    #dense later
    loc = Dense(50)(location_data)
    # scale up to image dimensions with linear activation
    n_nodes = image_shape[0] * image_shape[1] * 3
    loc = Dense(n_nodes)(loc)
    # reshape to additional channel
    loc = Reshape((image_shape[0], image_shape[1], 3))(loc)
    #concatenate images channel-wise
    merged = Concatenate()([in_src_image, in_target_image, loc])
    #C64
    ...
    #define model
    model = Model([in_src_image, in_target_image, location_data], patch_out)
    #compile model
...
def define_generator(image_shape=(256, 256, 3), n_classes=5):
    ...
    #concentanate inputs
    merged = Concatenate()([in_image, loc])
    #encoder model
    ...
    #define model
    model = Model([in_image, location_data], out_image)
    return model
...
def define_gan(g_model, d_model, image_shape):
    ...
    gen_out = g_model([in_src, location_data])
    #connect the source input and generator output to the discriminator input
    dis_out = d_model([in_src, gen_out, location_data])
    #src image as input, generated image and classification output
    model = Model([in_src, location_data], [dis_out, gen_out])
    #compile model
    opt = Adam(lr=0.0002, beta_1=0.5)
